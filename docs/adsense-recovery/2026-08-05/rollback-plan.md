...
| `audit: replace fixed flagship count with evidence quality gate` | 고정 8개 문구를 6~8개 권장 범위와 페이지별 3점 품질 계약으로 교체 | 이전 숫자 중심 판정과 `PARTIAL_INSUFFICIENT_EVIDENCE` 문구가 돌아옴 | 이 커밋을 revert한 뒤 `audit:adsense`와 문서 일관성 재검증 |
| `docs: finalize seven-flagship human review readiness` | 7개 독립성, 문의·개인정보·사람 검토 게이트 문서 | 최종 사람 검토 근거와 PASS Draft 판정이 사라짐 | 이 커밋을 revert하고 PR 설명도 이전 판정으로 되돌림 |
...
- FLAGSHIP 콘텐츠/URL: 새 글, 새 URL 또는 임의 승격이 없다. 품질 판정 코드와 문서만 되돌리면 된다.
