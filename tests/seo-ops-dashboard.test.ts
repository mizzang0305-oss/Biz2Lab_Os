...
test("SEO ops dashboard surfaces scheduler state and analytics empty states", () => {
  ...
  assert.equal(dashboard.scheduler.currentTopic, "metabase-dashboard-automation-for-small-business");
  assert.equal(dashboard.scheduler.nextTopic, "apache-superset-bi-dashboard-automation");
  assert.match(dashboard.scheduler.currentGate, /수동 배포 금지/);
  assert.equal(dashboard.scheduler.nextRequiredArtifact, "metabase-dashboard-automation-for-small-business-hero");
