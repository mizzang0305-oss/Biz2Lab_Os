...
test("Phase 2 content set has 30 public Korean posts and excludes drafts/noindex from sitemap", () => {
  const allPosts = getAllPosts();
  const publicPosts = getPublicPosts();
  const sitemapPosts = getSitemapPosts();
  const draftPosts = allPosts.filter((post) => post.frontmatter.draft);

  assert.equal(publicPosts.length, 30);
  assert.equal(publicPosts.filter((post) => post.category === "automation").length, 12);
