...
## Article Context
- articleTitle: 식당 사장님을 위한 주문 관리 자동화 구조
- categoryStyle: 소상공인 운영: 주문, 예약, 고객 메모, 리뷰 대응이 한 화면에서 정리되는 친근하지만 전문적인 매장 운영 워크플로
- visualDifferentiationHint: 리뷰 대응 상태를 중심 모티프로 사용하고, 좌하단에서 우상단으로 흐르는 대각선 프로세스 위에 작은 상태 표식을 놓는다.
...
## Image Brief
전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지

## Provider Prompt (Korean)
Biz2Lab 한국어 비즈니스 글 "식당 사장님을 위한 주문 관리 자동화 구조"에 사용할 대표 이미지. 사용자 설명: 전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지. 목표 톤: 차분하고 신뢰감 있는 프리미엄 Korean SaaS/editorial style. 주문, 예약, 고객 메모, 리뷰 대응이 한 화면에서 정리되는 친근하지만 전문적인 매장 운영 워크플로. 색상은 teal, forest green, slate, warm yellow. 실제 브랜드나 서비스 화면이 아닌 추상 업무 시각화. 글 상단 대표 이미지로 사용할 수 있게 한눈에 주제가 잡히는 넓은 16:9 에디토리얼 구도로 만든다. 리뷰 대응 상태를 중심 모티프로 사용하고, 좌하단에서 우상단으로 흐르는 대각선 프로세스 위에 작은 상태 표식을 놓는다. 핵심 업무 맥락이 한눈에 보이게 한다. 실제 로고, 사람 얼굴, 제품 사진은 넣지 않는다. 이미지 안의 글자는 최소화하고, 읽어야 하는 설명은 alt/caption/본문에 둔다. 1200:675 비율에 맞춘 깔끔한 편집 이미지, 충분한 여백, 광고 친화적이고 신뢰감 있는 구성.

## Provider Prompt (English)
Create a Biz2Lab hero visual for a Korean business article titled "식당 사장님을 위한 주문 관리 자동화 구조". Direction: 전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지. Style: order, reservation, customer, and review operations, practical owner dashboard, friendly but professional; premium SaaS/editorial look; teal, forest green, slate, warm yellow. 글 상단 대표 이미지로 사용할 수 있게 한눈에 주제가 잡히는 넓은 16:9 에디토리얼 구도로 만든다. Use abstract workflow modules and useful business-diagram structure, with minimal in-image text. No real logos, no people or faces, no product photography, no private data, no fake screenshots.
...
## Filename And Paths
- filename: unify-order-channels-hero.png
- rawPath: assets/images/raw/unify-order-channels-hero.png
- optimizedPath: public/images/posts/unify-order-channels-1200.webp

## Alt Text
전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지를 설명하는 소상공인 운영 이미지

## Caption
전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지를 안전한 로컬 이미지 제작용 브리프로 정리합니다.

## Manifest Draft
```json
{
  "id": "unify-order-channels-hero",
  "project": "biz2lab",
  "postSlug": "unify-order-channels",
  "usage": "hero",
  "src": "/images/posts/unify-order-channels-1200.webp",
  "rawPath": "assets/images/raw/unify-order-channels-hero.png",
  "altKo": "전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지를 설명하는 소상공인 운영 이미지",
  "captionKo": "전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지를 안전한 로컬 이미지 제작용 브리프로 정리합니다.",
  "width": 1200,
  "height": 675,
  "format": "webp",
...
## Article Update Plan
- 실제 이미지 승인 후 frontmatter heroImage 후보: /images/posts/unify-order-channels-1200.webp
- 실제 이미지 승인 후 frontmatter heroAlt 후보: 전화, 메시지, 플랫폼, 현장 주문이 하나의 식당 주문 상태 보드로 모이는 운영 구조 대표 이미지를 설명하는 소상공인 운영 이미지
...
- Save the raw image to assets/images/raw/unify-order-channels-hero.png.
- Optimize to public/images/posts/unify-order-channels-1200.webp only after manual review.
