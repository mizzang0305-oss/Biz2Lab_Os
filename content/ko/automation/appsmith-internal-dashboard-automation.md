...
데이터소스 자격 증명은 브라우저 코드에 넣지 않고 Appsmith의 서버 측 연결에만 보관해야 한다. 조회 쿼리와 수정 액션의 권한을 분리하고, 삭제·승인·결제 상태 변경처럼 되돌리기 어려운 버튼에는 재확인과 관리자 권한, 실행 로그를 함께 두어야 내부 화면이 우회 관리 도구로 변하지 않는다.
...
오늘은 조회 전용 목록 화면 한 개를 만들어 필요한 필드와 담당자를 확인한다. 첫 1주에는 테스트 데이터로 검색·필터·오류 상태를 검증하고, 1개월 안에는 수정 기능별 승인자와 감사 로그, 실패 시 수동 처리 경로가 준비된 액션만 단계적으로 연다.
