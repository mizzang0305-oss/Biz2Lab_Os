...
faq:
  - question: PocketBase를 바로 실운영 핵심 도구로 써도 되나요?
  ...
  - question: PocketBase를 자동화 파이프라인에 붙일 때 먼저 확인할 기준은 무엇인가요?
...
## PocketBase에서 먼저 풀어야 할 문제

PocketBase를 검토하는 이유는 새 도구를 하나 더 늘리기 위해서가 아닙니다. 반복되는 내부 업무를 줄이고, 운영자가 확인해야 하는 데이터와 승인 흐름을 한 화면에서 다룰 수 있는지 판단하기 위해서입니다.

## PocketBase 판단에 필요한 핵심 기준

핵심은 PocketBase 자체보다 연결 구조입니다. 데이터 원천, 권한, 승인 화면, 자동화 로그, 예외 처리 기준이 분리되어 있어야 실제 운영에서 안전하게 테스트할 수 있습니다.

## PocketBase 적용이 필요한 실제 업무 장면
...
## PocketBase 운영을 안전하게 만드는 구조

슈퍼유저 자격 증명을 애플리케이션에서 사용하지 말고 컬렉션별 list·view·create·update 규칙을 명시해야 한다. 파일의 크기와 유형을 제한하고 DB 파일 및 업로드 디렉터리를 함께 백업하며, 동시 사용자·데이터 크기·복구 시간 임계치를 정해 단일 인스턴스 구조를 넘어설 시점을 미리 판단해야 한다.

## PocketBase의 실패 위험과 방지책

가장 큰 리스크는 무료 오픈소스라는 이유로 운영 권한을 너무 빨리 넘기는 것입니다. 테스트 단계에서는 샘플 데이터, 제한 계정, 별도 로그, 백업 계획을 기준으로 검증해야 합니다.

## PocketBase 시작 순서: 오늘·1주·1개월

오늘은 로컬 테스트 컬렉션 하나에 인증 사용자와 비사용자의 접근 규칙을 적는다. 첫 1주에는 생성·수정·파일 업로드·백업 복원을 반복 검증하고, 1개월 안에는 실제 부하와 장애 복구 시간을 측정해 MVP 유지 또는 관리형 백엔드 이전 기준을 확정한다.
