...
n8n은 워크플로 자동화 구조를 배우고 내부 자동화 후보를 검토할 때 참고할 만한 도구입니다. 다만 Biz2Lab 관점에서는 무료 여부보다 라이선스, self-hosting 운영 책임, credential 관리, 고객-facing 사용 범위를 먼저 확인해야 합니다. 내부 리포트나 콘텐츠 소재 정리처럼 사람이 마지막에 확인하는 흐름부터 테스트하고, 고객 데이터·결제·메시지 발송이 들어가는 자동화는 별도 검토 뒤에 다루는 편이 안전합니다.

## n8n이 맞는 경우와 피해야 할 경우

| 구분 | 판단 기준 |
| --- | --- |
| 쓰면 좋은 경우 | RSS 감지, 내부 작업 로그, 콘텐츠 소재 정리, 샘플 데이터 기반 workflow 검증처럼 실패해도 되돌리기 쉬운 내부 자동화 |
| 피해야 할 경우 | 라이선스 범위가 확인되지 않았거나, 고객-facing 기능·결제·메시지·production DB write를 바로 연결하려는 경우 |
| 먼저 확인할 리스크 | Sustainable Use License, Enterprise 조건, self-hosting 보안, credential 보관, node/plugin 권한 |

## n8n 도입 전 확인 항목

- 공식 GitHub README와 LICENSE에서 현재 라이선스 표현을 확인합니다.
- 내부 업무 자동화인지, 고객에게 제공하는 기능인지, 자동화 backend인지 구분합니다.
- self-hosting 서버, 백업, SSL, 사용자 인증, 로그 보존 책임자를 정합니다.
- API key, OAuth token, webhook secret이 어디에 저장되는지 확인합니다.
- 처음에는 샘플 데이터와 내부 로그만 사용하고 외부 발송이나 DB 변경은 막아 둡니다.
