...
아래 화면은 운영 DB가 아닌 mock fixture입니다. 상단의 작업 lane에서 피킹·검수·상차가 각각 표시되고, 하단에는 일일 차이 확인과 “검수 미완료 시 상차 완료 차단” 경고가 보입니다. 화면이 보여 주는 설계 결정은 **검수 통과가 상차 완료의 선행 조건**이라는 점입니다.
...
소스 앱은 API가 없어도 `mock` fallback으로 실행되는 운영 콘솔입니다. 전용 clean worktree의 확인된 commit에서 브라우저를 띄웠고, 운영 API와 DB는 시작하지 않았습니다. 캡처 대상은 `section.screen.v262-page` 하나로 제한했으며 전체 화면 임의 캡처를 허용하지 않았습니다.
