...
faq:
  ...
  - question: 화면의 거래처와 금액은 실제 데이터인가요?
    answer: 아닙니다. mock fixture이며 첫 화면에는 합성 거래처명과 `FIXTURE-ORDER-001` 참조값만 주입했습니다. 검증 화면은 입력·상품·수량·가격 영역을 제외했습니다. 실제 단가, 한도, 재고와 주문 성과는 포함하지 않습니다.
---
...
아래 두 화면은 실제 거래처 주문이 아니라 mock fixture로 실행한 주문 작업대입니다. 첫 화면은 합성 거래처명과 `FIXTURE-ORDER-001` 참조값으로 주문 채널 필드만 보여 줍니다. 두 번째 390px 세로 화면은 입력·상품·수량·가격을 제외하고 재고 보류, 한도 보류와 검토 대기를 각각의 이유와 함께 보여 줍니다.
...
소스 앱은 `VITE_PORTAL_DATA_SOURCE=mock`에서 API 없이 fixture를 사용합니다. 캡처는 clean detached commit에서 실행했고 운영 DB, 환경 비밀값과 실제 거래처 정보는 연결하지 않았습니다. 첫 화면에는 합성 label만 입력했고 두 번째 화면은 검증 패널만 집중 캡처했습니다. body와 캡처 대상 텍스트에서 전화번호, 이메일, 사업자번호, 계좌 형태와 절대 경로를 검사했습니다.
